...
export default function Register() {
    ...
    const handleSubmit = async (event) => {
        ...
        try { await register(form.username, form.password); navigate('/login', { replace: true }) }
        catch (error) { setMessage(error.response?.data?.message || 'Impossible de créer ce compte.'); setStatus('failed') }
    }

    return (
        <Box className="editorial-grid">
            <Container maxWidth="sm">
                <Box sx={{ minHeight: 'calc(100vh - 77px)', display: 'flex', alignItems: 'center', py: 7 }}>
                    <Box component="form" onSubmit={handleSubmit} sx={{ width: '100%', bgcolor: 'background.paper', border: '1px solid', borderColor: 'primary.main', p: { xs: 3, sm: 5 }, boxShadow: { sm: '8px 8px 0 #14213D' } }}>
                        <Typography className="eyebrow">Premiers pas</Typography>
                        <Typography variant="h3" component="h1" sx={{ mt: 2, mb: 1 }}>Créez votre espace</Typography>
                        ...
                        <Box sx={{ display: 'grid', gap: 2.2 }}>
                            ...
                            <Button type="submit" variant="contained" color="secondary" size="large" disabled={status === 'loading'}>{status === 'loading' ? 'Création…' : 'Créer mon compte'}</Button>
                        </Box>
                        <Typography variant="body2" color="text.secondary" sx={{ mt: 3, textAlign: 'center' }}>Déjà inscrit ? <LinkMui component={Link} to="/login" color="primary" fontWeight={750}>Se connecter</LinkMui></Typography>
                    </Box>
                </Box>
            </Container>
        </Box>
